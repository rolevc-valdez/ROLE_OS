...
```json
{
  ...
  "about": {"version": "1.2.0", "commit": "e1dda55", "build_date": null, "license": "Proprietary"},
...
|--------------------------------|-------------------------------------------------------------|----------|
| `ROLE_OS_DB_PATH`               | `var/role_os/role_os.db`                 | Builder-generated knowledge database (read-only from the dashboard's perspective; regenerated by `builder.py`). Role OS 2.0 Phase 1 Task 2: previously defaulted into the bundled `samples/role_os_sample/00_SYSTEM/` fixture -- set this (or `ROLE_OS_WORKSPACE_DIR`) explicitly to use that fixture on purpose |
| `ROLE_OS_PROJECTS_DB_PATH`       | `var/role_os/role_os_projects.db`         | Project Intelligence database (dashboard-owned; schema and default workspaces are created automatically on first use) |
| `ROLE_OS_ADVISOR_DB_PATH`         | `var/role_os/role_os_advisor.db`           | AI Advisor recommendations database (dashboard-owned; schema created automatically on first use) |
| `ROLE_OS_IMPORTS_DB_PATH`         | `var/role_os/role_os_imports.db`           | ChatGPT Conversation Importer database (dashboard-owned; schema created automatically on first use) |
| `ROLE_OS_EXTRACTION_DB_PATH`      | `var/role_os/role_os_extraction.db`        | Knowledge Extraction database (dashboard-owned; schema created automatically on first use) |
...
| `ROLE_OS_WORKSPACE_DB_PATH`       | `var/role_os_dashboard/role_os_workspace.db`                | Workspace Adoption database (Discovery Engine Sprint 2, dashboard-owned): the cached Discovery scan plus the per-folder adopted/ignored overlay. Real personal data, not a fixture -- defaults under `var/`, not `samples/` |
| `ROLE_OS_ASSETS_DB_PATH`          | `var/role_os_dashboard/role_os_assets.db`                    | Assets OS database (Sprint C4, dashboard-owned): cached asset metadata (path/mtime/size-keyed) plus user overrides. Never contains file bytes -- the filesystem stays the source of truth |
| `ROLE_OS_ASSET_THUMBNAIL_CACHE_DIR` | `var/role_os_dashboard/asset_thumbnails`                   | Cached, resized preview thumbnails generated from scanned assets. Always safe to delete entirely; regenerated on next preview request |
| `ROLE_OS_ECOSYSTEM_DB_PATH`       | `var/role_os_dashboard/role_os_ecosystem.db`                 | Project Ecosystem Engine database (Sprint C8, dashboard-owned): manual relationship-override rows only -- relationships themselves are always recomputed fresh, never stored |
| `ROLE_OS_DISCOVERY_ROOT`          | this repository's own parent directory                      | The single folder the Discovery Engine scans by default (Discovery Engine Sprint 1). Superseded by `ROLE_OS_DISCOVERY_ROOTS` below when that's set; still used as its fallback, and still what an explicit `root` argument to `POST /workspace/rescan` overrides per-call regardless |
| `ROLE_OS_DISCOVERY_ROOTS`         | *(empty -- falls back to `ROLE_OS_DISCOVERY_ROOT`)*          | **Role OS 2.0 Phase 2 Task 2.1 (Multi-Root Discovery):** an optional, comma-separated list of folders to scan instead of one. Each is validated (must exist, must be a directory) and deduplicated (exact/case/slash duplicates and nested roots are dropped, keeping the outer root) before scanning; results from every surviving root are merged into one cached list, de-duplicated by the same identity every Workspace lookup already uses. Discovery is not adoption -- a project found this way is visible on the Workspace page but never auto-adopted, and Mission Control only ever reasons over the adopted portfolio regardless of how many roots are configured. See `docs/PHASE_2_MULTI_ROOT_DISCOVERY.md` |
| `ROLE_OS_DISCOVERY_EXTRA_EXCLUSIONS` | *(empty)*                                                 | Comma-separated extra folder names/glob patterns to exclude from a scan, layered on top of the Discovery Engine's own default exclusion list. Never an absolute path |
...
The first five databases above (`role_os.db`, `role_os_projects.db`,
`role_os_advisor.db`, `role_os_imports.db`, `role_os_extraction.db`) are
intentionally separate: the knowledge DB is regenerated wholesale each
time `builder.py` runs; the projects DB is mutated incrementally through
the `/pi/*` API; the advisor DB is written only by the recommendation
engine; the imports DB is written only by the `/import/*` API and CLI; the
extraction DB is written only by the `/extraction/*` API (reading
conversation content from the imports DB, but never writing back into
it). None of the five is ever clobbered by changes to another. The
Role OS 2.0 domains below them (Session, Workspace, Assets, Ecosystem)
follow the same "each domain owns its own SQLite file" rule -- see
`docs/RUNTIME_DATA_MAP.md` for the full current inventory of every
...
dashboard/
  app/
    ...
    discovery/                      # Discovery Engine domain (Role OS 2.0 Phase 1, Sprints 1-3; Phase 2 Task 2.1)
      scanner.py, detectors/, classifier.py, boundary/, health.py   # Read-only filesystem scan -> classify -> score
      roots.py                         # Phase 2 Task 2.1: validates/deduplicates multiple configured scan roots
      service.py                       # run_audit(): the one entry point every caller (CLI, Workspace) shares
    workspace/                      # Workspace Adoption domain (Role OS 2.0 Phase 1, Discovery Engine Sprint 2+)
      service.py                       # Orchestrates cached Discovery scans + the adopted/ignored user overlay
      db.py, identity.py, advisor.py, assets_index.py, activity.py, resume.py, portfolio.py, launcher.py
    project_context/                # ProjectContext domain (Sprint C1/C1B): the one canonical "project" shape
      builder.py                       # all_project_contexts()/build_project_context() -- every page reads through this
      health.py
    project_memory/                 # Project Memory (internal composition, no dedicated router -- composed into Project Context)
    project_ecosystem/              # Project Ecosystem Engine (Sprint C8): detected cross-project relationships
    impact_analysis/                # Impact Analysis (Sprint C9): "what else might this change affect"
    assets/                         # Assets OS domain (Sprint C4): shared filesystem asset index + previews
      service.py                       # request_scope(): the one shared per-request filesystem walk every domain reuses
    operational_intelligence/       # Operational Intelligence Engine (Sprint C6): the one canonical recommendation ranking
    executive_decision/             # Executive Decision Engine (Sprint C10): "what to work on today," layered on OI
    mission_control/                # Mission Control (Sprint C5): composes everything above into one GET /mission-control payload
      service.py                       # build_mission_control() -- pure composition, no new scoring
    explorer/                       # Explorer domain (workspace-wide search/filter view over discovered items)
    routers/
      ...
      workspace.py                                                     # Workspace Adoption API, namespaced /workspace (Discovery + adopted overlay)
      assets.py                                                        # Assets OS API, namespaced /assets
      project_context.py                                                # ProjectContext API -- the shared shape every project-detail-style page reads
      project_ecosystem.py                                               # Project Ecosystem API, namespaced /ecosystem
      mission_control.py                                                  # Mission Control API: one route, GET /mission-control
      dashboard.py                                                         # Dashboard v2 drill-down API, namespaced /dashboard (metrics behind Mission Control)
      explorer.py                                                           # Explorer API: workspace-wide search/filter over discovered items
    templates/
