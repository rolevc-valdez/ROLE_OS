...
| Sprint 7 | Dashboard | A new executive-summary sidebar page (separate from Home, which keeps covering the Project Intelligence/Advisor/Epic 3 Graph pipeline) over the Importer/Explorer/Extraction/Knowledge Graph/Advisor Search pipeline: ten summary cards (all read from the existing `GET /import/metrics`), Recent Activity, System Status, and Quick Actions to every related page. Two thin, additive endpoints (`GET /extraction/recent`, `GET /extraction/runs`); zero new storage, zero new calculation logic, zero AI. |
