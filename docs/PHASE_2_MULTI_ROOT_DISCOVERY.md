...
- `bolsa-de-trabajo` — `PROJECT_REGISTRY.md`'s listed path, `C:\Users\rolev\Documents\bolsa-de-trabajo`, does not exist; `Test-Path` confirms. Likely moved, renamed, or the registry entry is stale — a discrepancy to flag to Role, not something this task resolves.
...
- `SUPER-FACIL` and `bolsa-de-trabajo` (named in `NEXT_ACTIONS.md`/`PROJECT_REGISTRY.md` respectively) do not exist at their documented paths as of this validation — flagged above, not resolved; Role should confirm whether these were moved, renamed, or the source documents are stale.
