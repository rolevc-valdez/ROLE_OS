...
**Second historical-status note (added during Phase 2's Runtime Data Hygiene task, 2026-09-15):** `dashboard/var/role_os_dashboard/` — described throughout this document as the location holding the real, historically-accumulated workspace/session data — **has been deleted**. Its unique real content (5 real `adopted_projects` rows, the full `role_os_session.db`) was confirmed already present in canonical `var/role_os_dashboard/` before deletion (Task 3D's migration, 2026-09-10); every remaining difference (9 debug `adopted_projects` rows pointing at confirmed-nonexistent temp folders, and `asset_overrides` rows traced conclusively to pytest test fixtures in both copies) was proven to be debug/test data, not real user data. Full inventory, comparison, and deletion gate evidence: `docs/PHASE_2_RUNTIME_DATA_HYGIENE.md`. Every other finding below involving this path (the CWD-anchoring history, the Task 3D migration record) remains an accurate historical account of what existed at the time and is not rewritten.
