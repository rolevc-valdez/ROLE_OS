...
Normal startup's default runtime data root is var\role_os\ (empty until
you run the Builder against your own ChatGPT export, or otherwise place a
role_os.db there). To explore ROLE OS with the bundled demo data instead,
or to point at a real workspace the Builder already produced elsewhere,
set ROLE_OS_WORKSPACE_DIR to that folder (the one containing 00_SYSTEM\)
before running this launcher again -- e.g. the repository's own
samples\role_os_sample\ folder for the demo -- see INSTALLATION.md for
details.
"@
}
